...
Local operator controls live under `data/runs/live_control/`. Missing files mean all eligible games trade normally. Global auto-trade off, risk mode `kill`, or a per-game `abort` file blocks new execution. The webapp must display the execution default and risk mode and expose global default plus per-game abort/clear controls.
